import {
  FaFacebook,
  FaGoogle,
  FaInstagram,
  FaPhone,
  FaTelegram,
} from "react-icons/fa";
import { FaMapLocation } from "react-icons/fa6";
import CardsImg from "../../assets/credit-cards.webp";
import Logo from "../../assets/logo.png";
import { motion } from "framer-motion";

const Footer = () => {
  return (
    <footer className="bg-orange-500 pt-12 pb-8 text-white">
      <div className="flex justify-center">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-16">
          <motion.div
            initial={{ opacity: 0, y: 100 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{
              delay: 0.2,
              duration: 0.6,
            }}
            className="space-y-8"
          >
            <img src={Logo} alt="Logo Image" className="max-w-[100px] invert" />
            <div>
              <p className="flex items-center gap-2">
                <FaPhone />
                +1 (123) 456-7890
              </p>
              <p className="flex items-center gap-2 mt-2">
                {" "}
                <FaMapLocation /> Noida, Uttar Pradesh
              </p>
            </div>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 100 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{
              delay: 0.4,
              duration: 0.6,
            }}
            className="space-y-8"
          >
            <h1 className="text-3xl font-bold ">Quick Links</h1>
            <div className="flex gap-10">
              <div>
                <ul className="space-y-3">
                  <li>Home</li>
                  <li>About</li>
                  <li>Contact us</li>
                  <li>Privacy Policy</li>
                </ul>
              </div>
              <div>
                <ul className="space-y-3">
                  <li>Home</li>
                  <li>About</li>
                  <li>Contact us</li>
                  <li>Privacy Policy</li>
                </ul>
              </div>
            </div>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 100 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{
              delay: 0.6,
              duration: 0.6,
            }}
            className="space-y-8"
          >
            <h1 className="text-3xl font-bold">Follow us</h1>
            <div className="flex items-center cursor-pointer gap-3">
              <FaFacebook className="text-3xl hover:scale-110 duration-300" />
              <FaInstagram className="text-3xl hover:scale-110 duration-300" />
              <FaTelegram className="text-3xl hover:scale-110 duration-300" />
              <FaGoogle className="text-3xl hover:scale-110 duration-300" />
            </div>
            <div className="space-y-2">
              <p>Payment Methods</p>
              <img className="w-80" src={CardsImg} alt="" />
            </div>
          </motion.div>
        </div>
      </div>
      <p className="text-center mt-8 border-t-2 border-white/40 pt-8">
        Copyright &copy; 2025. All Rights Reserved by Fanta
      </p>
    </footer>
  );
};

export default Footer;
